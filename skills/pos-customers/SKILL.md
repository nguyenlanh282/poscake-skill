---
name: pos-customers
description: Tra cứu và quản lý khách hàng: list/get/create/update, level, điểm thưởng, ghi chú.
...
## Mục tiêu
Tra cứu và quản lý khách hàng: list/get/create/update, level, điểm thưởng, ghi chú.
...
## Cách gọi nhanh
Script gợi ý: `scripts/customers.sh`

Ví dụ (GET):
```bash
export POS_API_KEY="***"
export SHOP_ID="123"
bash scripts/customers.sh list "?page=1&page_size=20"
```

Ví dụ (WRITE):
```bash
export CONFIRM_WRITE=YES
cat payload.json | bash scripts/customers.sh create
```
...
- `/shops/{SHOP_ID}/customers/point_logs`
- `/shops/{SHOP_ID}/customers/{CUSTOMER_ID}/load_customer_notes`
- `/shops/{SHOP_ID}/customers/{CUSTOMER_ID}/create_note`
- `/shops/{SHOP_ID}/customer_levels`
